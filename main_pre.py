...
# Training settings
parser = argparse.ArgumentParser(description="Train lightweight vdsr model in AIL")
parser.add_argument("--width", type=int, default=13, help="width of network, num of feature maps")
...
def save_checkpoint(model):
    model_out_path = "model/" + "pre_vdsr_f{}.pth".format(opt.width)
    state = {"model": model}
    if not os.path.exists("model/"):
        os.makedirs("model/")

    torch.save(state, model_out_path)
    print("lightweight model has been saved to {}".format(model_out_path))
